...
with tabs[1]:
    ...
    with col1:
        ...
        # NEW: Audio recorder in the second column with FIXED logic
        with input_col2:
            ...
            # Handle audio transcription - FIXED: properly handle new vs processed audio
            if audio_bytes and api_key_available:
                # Check if this is completely new audio (different from last processed audio)
                if audio_bytes != st.session_state.last_audio_bytes:
                    # Reset audio_processed for new audio
                    st.session_state.audio_processed = False
                    
                # Only process if it's new audio and hasn't been processed yet
                if (audio_bytes != st.session_state.last_audio_bytes and 
                    ...
                    try:
                        ...
                        with tempfile.NamedTemporaryFile(suffix=".wav", delete=False) as temp_audio:
                            ...
                            if transcribed_text:
                                # Update session state - mark this specific audio as processed
        ...
        # Handle Submit button action - FIXED audio state management
            ...
            with st.spinner("Searching documents and generating answer..."):
                try:
                    ...
                    if hasattr(st.session_state, 'rag_system') and st.session_state.rag_system.is_initialized:
                        ...
                        # FIXED: Don't keep audio_processed as True after submission
                        # Reset audio_processed to allow new recordings to be processed
                        st.session_state.audio_processed = False
                        # Keep last_audio_bytes to prevent re-processing the same audio
                        ...
                        # Reset audio_processed to allow new recordings
                        st.session_state.audio_processed = False
        ...
        # Handle Clear chat button action - FIXED: Properly reset audio state
        if clear_btn:
            st.session_state.chat_history = []
            st.session_state.current_question = ""
            st.session_state.transcribed_text = ""
            # FIXED: Only reset audio state when explicitly clearing chat
            st.session_state.audio_processed = False
            st.session_state.last_audio_bytes = None
            st.session_state.input_key_counter += 1
    ...
    # Second column - Settings and Source Documents
    with col2:
        # Settings section
